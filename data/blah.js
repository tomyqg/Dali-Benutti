...
function showLoadingScreen() {
    ...
    var loadingDiv = $("<div id='loadingDiv'></div>");

    // Add a loading message to the div
...
function renderAccordion() {
    ...
    $(".light-buttons button").on("click", function() {
        ...
        // send the request for detail
        var socketData = {
            command: "fetchDaliDetail",
            sa: lightShortAddress
        };
...
function showDaliDetail(shortAddress) {
    var daliLightData = Dlights.find(function(light) {
        return light.shortAddress === shortAddress;
    });
    var formHTML = '<form align="center"><input type="hidden" name="command" value="saveLightData"><input type="hidden" name="shortAddress" value="' + daliLightData.shortAddress + '">';
        ...
        if (data.hasOwnProperty("command")) {
            switch (data.command) {
                case "lights":
                    ...
                    break;
                case "refresh":
                    // refresh page after (re)connect.
                    location.reload();
                    break;
                case "dalidetail":
                    // find the light data from the Dlights array
                    var shortAddress = data.data[0].shortAddress;
                    var daliLightData = Dlights.find(function(light) {
                        return light.shortAddress === shortAddress;
                    });

                    // update the light data with the fetched details
                    var fetchedData = data.data[0];
                    daliLightData.name = fetchedData.name;
                    daliLightData.room = fetchedData.room;
                    daliLightData.minLevel = fetchedData.minLevel;
                    daliLightData.maxLevel = fetchedData.maxLevel;
                    daliLightData.groups = fetchedData.groups;
                    daliLightData.sceneLevels = fetchedData.sceneLevels;
                    daliLightData.failLevel = fetchedData.failLevel;
                    daliLightData.phyMinLevel = fetchedData.physmin;
                    daliLightData.fadeTime = fetchedData.fadeTime;
                    daliLightData.fadeRate = fetchedData.fadeRate;
                    daliLightData.powerOnLevel = fetchedData.powerOnLevel;

                    // remove the loading screen and show the updated data
                    hideLoadingScreen();
                    showDaliDetail(shortAddress);
                    break;
                case "levels":
